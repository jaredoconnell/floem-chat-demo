...
// ---------------------------------------------------------------------------
// Snap-to-right layout helpers
// ---------------------------------------------------------------------------

/// Recompute `target_x` for all docked panes so they pack from the right
/// edge of the window with `PANE_SPACING` gaps, ordered by their current
/// `target_x` (rightmost = smallest target_x, positioned furthest right).
///
/// Undocked (floating) panes are ignored.
fn recompute_dock_targets(panes: &mut [PaneState], window_width: f64) {
    // Collect indices of docked panes sorted by their current target_x
    ...
    let mut docked: Vec<usize> = panes
        ...
        .map(|(i, _)| i)
        .collect();
    docked.sort_by(|&a, &b| panes[a].target_x.partial_cmp(&panes[b].target_x).unwrap());

    // Pack from the right edge leftward
    let mut cursor = window_width;
    for &i in docked.iter().rev() {
...
/// slot in among the other docked panes, then re-pack so everything else
/// shifts to make room.
fn recompute_targets_during_drag(panes: &mut [PaneState], drag_id: usize, window_width: f64) {
    // Collect docked panes excluding the dragged one, sorted by target_x
    let mut others: Vec<usize> = panes
        ...
        .map(|(i, _)| i)
        .collect();
    others.sort_by(|&a, &b| panes[a].target_x.partial_cmp(&panes[b].target_x).unwrap());

    let drag_idx = panes.iter().position(|p| p.id == drag_id);
    let Some(drag_idx) = drag_idx else { return };
    let drag_pane = &panes[drag_idx];

    if !drag_pane.docked {
        // Dragged pane is floating; just re-pack the remaining docked ones
        let mut cursor = window_width;
    ...
    // Find the insert position: the dragged pane's center determines
    // where it belongs in the sorted order.
    let drag_center = drag_pane.x + drag_pane.width / 2.0;
...
fn app_view() -> impl IntoView {
    ...
    let toolbar = toolbar(panes, next_pane_id, window_size, dragging, animating);
    ...
    Stack::new((pane_area, toolbar))
        ...
        .on_event_cont(
            ...
            move |_, size: &floem::kurbo::Size| {
                ...
                if (old.0 - size.width).abs() > 1.0 {
                    panes.update(|p| recompute_dock_targets(p, size.width));
                    start_animation(panes, dragging, animating);
                }
            },
        )
        ...
        .on_event_cont(listener::PointerUp, move |_, _| {
            ...
            if resizing.get_untracked().is_some() {
                let (ww, _) = window_size.get_untracked();
                panes.update(|p| recompute_dock_targets(p, ww));
                resizing.set(None);
                start_animation(panes, dragging, animating);
                return;
            }
            ...
            if let Some(drag) = dragging.get_untracked() {
                if drag.moved {
                    ...
                    // Click without movement: toggle collapsed
                    panes.update(|p| {
                        if let Some(pane) = p.iter_mut().find(|ps| ps.id == drag.pane_id) {
